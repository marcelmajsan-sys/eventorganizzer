...
export async function GET(request: NextRequest) {
  ...
  for (const projectId of ["2026", "2025"] as const) {
      ...
      for (const benefit of benefits ?? []) {
        ...
        try {
          const { error: sendError } = await resend.emails.send({
            from: FROM_EMAIL, to: recipient, subject, html,
          });
    ...
    for (const benefit of allBenefits ?? []) {
      ...
      try {
        // Alert šaljemo SAMO za benefite koji su TEK SADA prešli u overdue —
        // update vraća retke samo ako je status stvarno promijenjen.
        ...
        if (sponsor?.name) {
          const { error: alertError } = await sendOverdueAdminAlert(
            sponsor.name,
            benefit.benefit_name,
            benefit.deadline,
            Math.abs(days)
          );
          if (alertError) results.errors++;
        }
      } catch (e) {
        results.errors++;
      }
