...
function CellContent({ value, isHighlight }: { value: Cell; isHighlight: boolean }) {
  if (value === true) {
    return (
  ...
  // string — e.g. "2 VIP ulaznice", "+500 € opcija", "keynote slot"
  return (
    <span
...
export default function PortalCollaborationOptions() {
  return (
                      ...
                      >
                        <p
                          className={`font-bold text-sm leading-tight ${
                            pkg.highlight ? "text-blue-700" : "text-gray-800"
                          }`}
                        >
                          {pkg.name}
                        </p>
                        <p
                          className={`text-[10px] mt-0.5 font-semibold uppercase tracking-wide ${
                            pkg.highlight ? "text-blue-400" : "text-gray-400"
                          }`}
                        >
                          {pkg.slots}
                        </p>
                        <p
                          className={`text-sm font-bold mt-1.5 ${
                            pkg.highlight ? "text-blue-700" : "text-gray-700"
                          }`}
                        >
                          {pkg.price}
                        </p>
                        ...
                        {row.values.map((val, vi) => (
                          <td
                            ...
                            className={`py-3 px-1.5 text-center align-middle ${
                              PACKAGES[vi].highlight ? "bg-blue-50/40" : ""
                            }`}
                          >
                            <CellContent value={val} isHighlight={PACKAGES[vi].highlight} />
                          </td>
                        ))}
                      </tr>
                    ))}
