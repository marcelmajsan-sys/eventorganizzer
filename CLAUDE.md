...
- **Admin "Podijeli dokument" (rujan 2026.)**: gumb u zaglavlju `/admin/sponsors` (`ShareDocumentModal.tsx`, uz "Preuzmi kontakte") — jedan upload (npr. tlocrt) ide u `sponsor-files/shared/{timestamp}_{ime}` (ime sanitizirano za storage ključ, original ostaje u `files.filename`), a server action `shareDocumentWithSponsors` (`actions/sharedDocuments.ts`, `requireAdmin`, `createAdminClientForProject`) upiše po jedan `files` red po partneru. Opseg: **Svi potvrđeni partneri (klijenti)** (default, `confirmed_new`/`confirmed_returning`) / Trenutno filtrirani (samo kad je lista filtrirana) / Svi partneri. Idempotentno — partner koji već ima isti `storage_url` se preskače (vraća `shared`/`skipped`). Partneri dokument vide u tabu "Dokumenti" na `/portal/sponsor`; brisanje ide kroz postojeći `FileUploadSection` na profilu partnera (shared/ objekt se ne briše)
